...
if (!isset($_GET['Cajero']) || empty($_GET['Cajero'])) {
    $error_message = "Error: No se especificó el id del usuario.";
} else {
    $usuario_id = $_GET['Cajero'];

    if (!isset($conn) || $conn->connect_error) {
        $error_message = "Error crítico de conexión a la base de datos: " . ($conn->connect_error ?? "Error desconocido.");
    } else {
        $sql = "UPDATE usuario SET estado_usuario = 2 WHERE id_usuario = ?";
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                $error_message = "Error al intentar eliminar el usuario: " . $stmt->error;
            }
            $stmt->close();
        } else {
