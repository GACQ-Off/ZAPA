...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    $error_message = "Error: No se especificó el ID de la categoría.";
} else {
    ...
    if (!isset($conn) || $conn->connect_error) {
        ...
        $sql = "UPDATE categoria SET estado_categoria = 2 WHERE id_categoria = ?";
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                $error_message = "Error al intentar eliminar la categoría: " . $stmt->error;
            }
            $stmt->close();
        } else {
