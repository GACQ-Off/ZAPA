...
if (!isset($_GET['id_gasto']) || empty($_GET['id_gasto'])) {
    $error_message = "Error: No se especificó el id del gasto.";
} else {
    $gasto_id = $_GET['id_gasto'];

    if (!isset($conn) || $conn->connect_error) {
        $error_message = "Error crítico de conexión a la base de datos: " . ($conn->connect_error ?? "Error desconocido.");
    } else {
        $sql = "UPDATE gastos SET estado_gasto = 2 WHERE id_gastos = ?";
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                $error_message = "Error al intentar eliminar el gasto: " . $stmt->error;
            }
            $stmt->close();
        } else {
