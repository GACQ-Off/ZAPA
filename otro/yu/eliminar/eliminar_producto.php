...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    $error_message = "Error: No se especificó el ID del producto.";
} else {
    $producto_id = $_GET['id'];
    ...
    if (!isset($conn) || $conn->connect_error) {
        ...
        $sql = "UPDATE producto SET estado_producto = 2 WHERE id_pro = ?";
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                $error_message = "Error al intentar eliminar el producto: " . $stmt->error;
            }
            $stmt->close();
        } else {
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        <?php if ($show_modal): ?>
            ...
            setTimeout(function() {
                window.location.href = "../listas/lista_productos.php";
            }, 3000);
