...
if (!isset($_GET['Empleado']) || empty($_GET['Empleado'])) {
    $error_message = "Error: No se especificó el ID del empleado.";
} else {
    ...
    if (!isset($conn) || $conn->connect_error) {
        ...
        $sql = "UPDATE empleado SET estado_empleado = 2 WHERE cedula_emple = ?";
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                $error_message = "Error al intentar eliminar el empleado: " . $stmt->error;
            }
            $stmt->close();
        } else {
