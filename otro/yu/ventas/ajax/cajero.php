...
function abrirTicket(idVenta) {
    if (idVenta && idVenta > 0) {
        const url = `ticket.php?id=${idVenta}`;
...
// CÓDIGO CORREGIDO:
// Esta función se ejecutará después de que el servidor procese la venta exitosamente
<?php if (isset($_SESSION['ticket_id']) && $_SESSION['ticket_id'] > 0): ?>
    document.addEventListener('DOMContentLoaded', function() {
        abrirTicket(<?php echo $_SESSION['ticket_id']; ?>);
        // Limpiar la variable de sesión después de usar
