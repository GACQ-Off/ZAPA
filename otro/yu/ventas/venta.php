                    ...
                    <tbody id="salesTableBody">
                        <?php while($row = $result->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td class="actions">
                                    <a href="detalle.php?id_venta=<?php echo htmlspecialchars($row["ID_Venta"]); ?>" title="Ver detalles de esta venta">Ver Detalles</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
                ...
                if (sales.length > 0) {
                    sales.forEach(sale => {
                        ...
                        row.innerHTML = `
                            ...
                            <td class="actions">
                                <a href="detalle.php?id_venta=${sale.ID_Venta}" title="Ver detalles de esta venta">Ver Detalles</a>
                            </td>
                        `;
            ...
            async function fetchSales(page = 1) {
                ...
                try {
                    const response = await fetch(`venta.php?${params.toString()}`);
                    if (!response.ok) {
