...
class PDF extends FPDF {
    function Header() {
        ...
        if (file_exists($logoPath)) {
            $this->Image($logoPath, 36, 5, 18);
        }
        $this->Ln(22);
        $this->SetFont('Arial', 'B', 12);
        $this->Cell(90, 5, utf8_decode("RECIBO DE COMPRA"), 0, 1, 'C');
        $this->Ln(1);
    }
...
class imprimirFactura {
    ...
    public function traerImpresionFactura() {
        ...
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(80, 5, $nombreEmpresa, 0, 1, 'C');
        $pdf->Cell(80, 5, utf8_decode("RIF: ") . $rifEmpresa, 0, 1, 'C');
        $pdf->MultiCell(80, 4, $direccionEmpresa, 0, 'C');
        $pdf->Cell(80, 5, utf8_decode("Tel: ") . $telefonoEmpresa, 0, 1, 'C');

        $pdf->Ln(2);
        $pdf->SetDrawColor(200,200,200);
        $pdf->Line(5, $pdf->GetY(), 85, $pdf->GetY());
        $pdf->Ln(2);
        ...
        $pdf->Cell($anchoCodigo, 5, 'Código', 1, 0, 'C', true);
        $pdf->Cell($anchoDescripcion, 5, 'Descripción', 1, 0, 'C', true);
        ...
        $pdf->Ln(2);
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Cell(80, 5, str_repeat('-', 45), 0, 1, 'C');

        $anchoEtiquetaTotal = 40;
        $anchoMontoTotal = 35;
