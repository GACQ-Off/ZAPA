...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell($this->table_width, 10, utf8_decode('REPORTE TOTAL DE VENTAS'), 0, 1, 'C', true); // Usa el ancho de la tabla
    ...
    function Footer()
    {
        ...
        // Usamos GetY() para posicionar la línea de forma dinámica, más robusto
...
// SetTopMargin a un valor razonable para que el contenido se vea.
$pdf->SetTopMargin(50); 
...
$pdf->Cell(55, 10, utf8_decode('Cliente'), 1, 0, 'C', 1); // Ancho ajustado a 55
$pdf->Cell(35, 10, utf8_decode('Vendedor'), 1, 0, 'C', 1); // Ancho ajustado a 35
$pdf->Cell(28, 10, utf8_decode('Fecha'), 1, 0, 'C', 1); // Ancho ajustado a 28
$pdf->Cell(37, 10, utf8_decode('Total (Bs)'), 1, 1, 'C', 1); // Ancho ajustado a 37
// Suma de anchos: 12 + 28 + 55 + 35 + 28 + 37 = 195mm (misma que reporte diario)

$ventas = ControladorVentas::ctrMostrarVentas(null,null,1); // Se mantiene la lógica original para obtener todas las ventas
...
if (is_array($ventas) && !empty($ventas)) {
    for ($i = 0; $i < count($ventas); $i++) {
        ...
        // Eliminado $pdf->Ln(0.5) para una tabla más compacta
    }
} else {
    $pdf->SetX($pdf->table_start_x); // POSICIÓN DEL MENSAJE "NO SE ENCONTRARON VENTAS" DESPLAZADA
    $pdf->Cell($pdf->table_width, 10, utf8_decode('No se encontraron ventas en el sistema.'), 1, 1, 'C');
}
...
$logData = array(
    ...
    "description" => "Se generó un reporte Total de Ventas en formato PDF. Total: " . number_format($totalCompleto, 2, ',', '.') . " Bs.",
