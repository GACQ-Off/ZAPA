...
// ---------------------------------------------------------------------------------
// 1. INCLUSIÓN DE CONTROLADORES CON RUTA CORREGIDA (../../)
// ---------------------------------------------------------------------------------

require_once "controladores/compras.controlador.php"; // ¡El controlador principal!
require_once "controladores/configuracion.controlador.php"; // Necesario para la Tasa de Cambio
// ---------------------------------------------------------------------------------
// 2. LÓGICA PHP PARA GENERAR OPCIONES DE PRODUCTO (Necesario para ambos selects)
// ---------------------------------------------------------------------------------
...
// Llama al controlador de productos y genera el array de datos
...
if (is_array($productos_data) && !empty($productos_data)) {
    foreach ($productos_data as $producto) {
        ...
        $opciones_productos_html .= '<option
            ...
            data-precio-compra-bs="'.$producto["precio_compra"].'"
        >
            '.$texto_visible.'
        </option>';
    }
...
// Define y escapa la variable para que JavaScript la pueda leer.
$opcionesProductosHTML = addslashes($opciones_productos_html); 
?>
                        ...
                        <div class="row">
                            <div class="col-xs-6">
                                <div class="form-group">
                                    <label>Producto (Temporalmente en lugar de Proveedor)</label>
                                    <select class="form-control" name="nuevoProveedor" id="nuevoProveedor" required>
                                        <option value="">Seleccionar producto</option>
                                        <?php
                                            // Usamos los datos de productos para llenar este select temporal
                                            foreach ($productos_data as $producto) {
                                                $texto_visible = $producto["codigo"] . ' - ' . html_entity_decode($producto["descripcion"], ENT_QUOTES, 'UTF-8');
                                                
                                                // El 'value' es el código del producto
                                                echo '<option value="'.$producto["codigo"].'">'.$texto_visible.'</option>';
                                            }
                                        ?>
                                    </select>
                                </div>
...
<script>
    ...
    // =============================================
    // 2. FUNCIÓN PARA CALCULAR TOTALES Y JSON
    // =============================================
    ...
    function actualizarEventosFila(fila) {
        ...
        selectProducto.addEventListener('change', function() {
            // Lee el data-precio-compra-bs del option seleccionado
        ...
        // **OPCIONAL**: Si usas Select2 u otro plugin, inicialízalo aquí
    ...
    // Cargar la primera fila automáticamente al abrir el modal (simula el click)
