...
static public function mdlIngresarConfiguracion($tabla, $datos){
    ...
    $stmt->bindParam(":tipo_rif", $datos["tipo_rif"], PDO::PARAM_STR); // Nuevo
    $stmt->bindParam(":num_rif", $datos["num_rif"], PDO::PARAM_STR);   // Nuevo

    if($stmt->execute()){
        return "ok";
    } else {
    	print_r($stmt->errorInfo()); // ← MOSTRAR ERROR
        return "error";
    }
