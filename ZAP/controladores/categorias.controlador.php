...
static public function ctrBorrarCategoria(){
	...
	if(isset($_GET["idCategoria"])){
		...
		if($respuesta == "ok"){
            ...
            // ⚠️ ELIMINACIÓN BLOQUEADA POR ON DELETE RESTRICT
            $empleadoCedula = (isset($_SESSION["cedula"])) ? $_SESSION["cedula"] : "Sistema/Desconocido";
            $logData = array(
                "event_type" => "Eliminación Categoría Restringida",
                "description" => "Intento de eliminar categoría '{$nombreCategoriaBorrada}' (ID: {$datos}) fallido: está relacionada con otros registros.",
                "employee_cedula" => $empleadoCedula,
                "affected_table" => $tabla,
                "affected_row_id" => $datos
            );
			...
			echo'<script>
				swal({
					...
					text: "Está siendo utilizada por otros registros.",
					showConfirmButton: true,
					confirmButtonText: "Cerrar"
				}).then(function(result){
					if (result.value) {
            ...
            $logData = array(
                ...
                "description" => "Error inesperado al intentar eliminar categoría (ID: {$datos}).",
                "employee_cedula" => $empleadoCedula,
                "affected_table" => $tabla,
                "affected_row_id" => $datos
            );
