...
if (empty($_POST['_Material'])) {
    die("Filtro de Material no especificado.");
}

$id_material = (int)filter_var($_POST['_Material'], FILTER_SANITIZE_NUMBER_INT);

$sql_trapiche = "SELECT * FROM trapiche LIMIT 1";
$trapiche_data = $bd->obtenerRegistro($sql_trapiche, '', []);
$datos_trapiche = $trapiche_data[0] ?? [];

$sql_entidad = "SELECT nombre_material FROM material WHERE id_material = ?";
$entidad_data = $bd->obtenerRegistro($sql_entidad, 'i', [$id_material]);
$nombre_entidad = $entidad_data[0]['nombre_material'] ?? 'Material Desconocido';
...
SELECT 
    ...
    ";
    $obras = $bd->obtenerRegistro($sql_obras, 'i', [$id_material]);
    
    $pdf = new PDF();
    $pdf->setTrapicheData($datos_trapiche);
$pdf->setReportTitle("Reporte de Obras por Material: {$nombre_entidad} (ID: {$id_material})"); 
$pdf->AliasNbPages();
$pdf->AddPage('L'); 
...
if (!empty($obras)) {
    ...
    $pdf->SetFillColor($fill ? 240 : 255);
    $pdf->Cell(array_sum($w), 6, utf8_decode("No se encontraron obras para este material."), 1, 1, 'C', true);
}
$pdf->Cell(array_sum($w), 0, '', 'T'); 
$pdf->Output('I', "Reporte_Obras_Material_{$id_material}.pdf");
?>
F